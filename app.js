...
.map(s => {
const typeScore = calculateTypeScore(s, globalAvgScore, globalAvgRating, maxCount);
...
// 10 首以下偏不穩，40 首以上視為很穩；和 v0.1.15 分類適性公式一致。
...
$("summaryCards").innerHTML = cards.map(([label, value]) => `     <div class="summary-card">       <div class="label">${escapeHtml(label)}</div>       <div class="value">${escapeHtml(value)}</div>     </div>
  `).join("");
...
tbody.innerHTML = enriched.map((s, i) => `     <tr>       <td>${i + 1}</td>       <td><strong>${escapeHtml(s.type)}</strong></td>       <td class="score">${s.hasData ? s.typeScore.toFixed(1) : ""}</td>       <td>${s.hasData ? s.radarValue.toFixed(2) : "0.00"}</td>       <td>${s.hasData && s.avgScore ? Math.round(s.avgScore).toLocaleString() : "-"}</td>       <td>${s.hasData && s.avgRating ? s.avgRating.toFixed(2) : "-"}</td>       <td>${s.count}</td>       <td class="tags">${escapeHtml((s.representativeSongs || []).join("、"))}</td>     </tr>
  `).join("");
}
...
svg.innerHTML = `     <text x="24" y="34" class="radar-caption">分類雷達圖：只顯示有資料的分類，依分類適性分數縮放到 0.00 ～ 5.00。</text>
    ${rings}
    ${axes}     <polygon class="radar-polygon" points="${polygonPoints}"></polygon>
    ${points}
    ${labels}
  `;
}
...
return recommendations.map((r, i) => `     <tr>       <td>${i + 1}</td>       <td><strong>${escapeHtml(r.song)}</strong></td>       <td>${escapeHtml(r.difficulty)}</td>       <td>${r.constant.toFixed(1)}</td>       <td class="score">${r.recommendScore.toFixed(1)}</td>       <td>${escapeHtml(r.mainType)}</td>       <td class="tags">${escapeHtml(r.subTypesText)}</td>       <td class="tags">${escapeHtml(r.skillTags || "")}</td>       <td>${escapeHtml(r.playerRecordText)}</td>       <td>${escapeHtml(r.targetScoreText)}</td>       <td class="reason">${escapeHtml(r.reason)}</td>     </tr>
  `).join("");
}
